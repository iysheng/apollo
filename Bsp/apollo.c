...
ADC_HandleTypeDef IADC,ICEKONG;
...
void TEMP_init(void)
{
  ...
  IADC.Instance = ADC1;
  IADC.Init.ClockPrescaler = ADC_CLOCK_SYNC_PCLK_DIV2;//APB2=108MHz
  IADC.Init.Resolution = ADC_RESOLUTION_12B;
  IADC.Init.ScanConvMode = ENABLE;
  IADC.Init.ContinuousConvMode = DISABLE;
  IADC.Init.DiscontinuousConvMode = DISABLE;
  IADC.Init.DataAlign = ADC_DATAALIGN_RIGHT;
  IADC.Init.NbrOfConversion = 1;
  IADC.Init.DMAContinuousRequests = DISABLE;
  IADC.Init.EOCSelection = ADC_EOC_SEQ_CONV;
  HAL_ADC_Init(&IADC);
  Iconfig.Channel = ADC_CHANNEL_TEMPSENSOR;
  Iconfig.Rank = 1;
  Iconfig.SamplingTime = ADC_SAMPLETIME_480CYCLES;
  HAL_ADC_ConfigChannel(&IADC,&Iconfig);
}
*/
...
void HAL_ADC_MspInit(ADC_HandleTypeDef* hadc)
{
  ...
  //HAL_GPIO_Init(GPIOA,&GPIO_Initure);//PA4
...
{
  ITIMX->Instance=TIM5;
  ITIMX->Init.Prescaler=107;//107--精度达到1us
  ITIMX->Init.CounterMode=TIM_COUNTERMODE_UP;
  ITIMX->Init.Period=0xffffffff;
  ITIMX->Init.ClockDivision=TIM_CLOCKDIVISION_DIV1;
  HAL_TIM_IC_Init(ITIMX);
  IConfig->ICPolarity=TIM_ICPOLARITY_RISING;
  IConfig->ICPrescaler=TIM_ICPSC_DIV1;
  IConfig->ICFilter=0;
  IConfig->ICSelection=TIM_ICSELECTION_DIRECTTI;
  HAL_TIM_IC_ConfigChannel(ITIMX,IConfig,IChannel);//TIM_CHANNEL_1
  HAL_TIM_IC_Start_IT(ITIMX,IChannel);//开启中断
}
